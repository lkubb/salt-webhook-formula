...
We will see a quick setup to configure the ``webhook`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``webhook`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``webhook`` formulas under ``/srv/salt/webhook/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/webhook/parameters/dns:domain/
    ln -s dns:domain /srv/salt/webhook/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/webhook/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/webhook/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``webhook`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``webhook`` formulas under ``/srv/salt/webhook/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/webhook/parameters/roles
...
We create a configuration for the role ``webhook/server`` in ``/srv/salt/webhook/parameters/roles/webhook/server.yaml``:
...
We create another configuration for the role ``webhook/client`` in ``/srv/salt/webhook/parameters/roles/webhook/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles webhook/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles webhook/client
...
.. code-block:: console
    ...
    mv /srv/salt/webhook/parameters/dns:domain/  '/srv/salt/webhook/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-webhook-as-json:
      test.nop:
        - name: {{ webhook | json }}
...
Here is an example based on `template-formula/webhook/config/file.sls`_:
    ...
    webhook-config-file-file-managed:
      file.managed:
        ...
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='webhook-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ webhook.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``webhook`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ webhook['winner'] }}
...
.. _template-formula/webhook/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/webhook/config/file.sls
